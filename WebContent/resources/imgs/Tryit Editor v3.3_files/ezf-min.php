...
function execute_ezuxMouseOut(e) {
    ...
    if (!from) ezux_callEzUnloadEvent('mouseout');
}
ez_attachEvent(window, 'pagehide', execute_ezuxPageHide);
ez_attachEvent(window, 'unload', execute_ezuxUnload);
ez_attachEvent(window, 'beforeunload', execute_ezuxBeforeUnload);
ez_attachEvent(window, 'blur', execute_ezuxBlur);
ez_attachEvent(window, 'visibilitychange', execute_ezuxVisiblityChanged);
ez_attachEvent(document, 'mouseout', execute_ezuxMouseOut);

var ezUnloadTime = null;
function ezux_callEzUnloadEvent(event) {
    var currentTime = (new Date).getTime();
    if (ezUnloadTime == null || currentTime > (ezUnloadTime + 3000)) {
        // var url = "/utilcave_com/inc/t.php?event=" + event + "&url=" + encodeURI(location.href);
        // var image = new Image();
        // image.src = url;
        ezUnloadTime = currentTime;
    }
}var ezuxo = ezuxo || {};

ezuxo.EzUX = function () {
    this.px = new EzoicPixel();
    if (typeof _ezaq === "undefined" || !_ezaq.hasOwnProperty("page_view_id")) {
        return;
    }
    this.pvID = _ezaq["page_view_id"];
};

ezuxo.EzUX.prototype.Init = function () {
...
ezuxo.EzUX.prototype.storeLocalTime = function() {
    var now = new Date();
    ...
    this.px.AddPVPixel(this.pvID, [(new EzoicPixelData('t_local_date', localDate))]);
    this.px.AddPVPixel(this.pvID, [(new EzoicPixelData('t_local_hour', localHour))]);
    this.px.AddPVPixel(this.pvID, [(new EzoicPixelData('t_local_day_of_week', localDay))]);
    this.px.AddPVPixel(this.pvID, [(new EzoicPixelData('t_local_timezone', localTZ))]);
};

ezuxo.EzUX.prototype.storeSizes = function () {
    this.px.AddPVPixel(this.pvID, [(new EzoicPixelData('device_width', screen.width))]);
    this.px.AddPVPixel(this.pvID, [(new EzoicPixelData('device_height', screen.height))]);
};

var ezux = new ezuxo.EzUX();
ezux.Init();
